...
export class NWKHandler {
                ...
                try {
                    ...
                    if (entry.recentLQAs.length > 0) {
                        ...
                        if (!Number.isNaN(avgLQA)) {
                            ...
                            const lqaPenalty = ((255 - avgLQA) / 36) | 0;
            ...
            if (triggerRepair || entry.failureCount >= CONFIG_NWK_ROUTE_MAX_FAILURES) {
                ...
                for (const [addr16, routeEntries] of this.#context.sourceRouteTable) {
                    ...
                    for (let i = 0; i < routeEntries.length; i++) {
                        ...
                        if (!entry.relayAddresses.includes(destination16)) {
     ...
     * - ✅ Costs the hop at max(incoming, outgoing) per #3.6.4.1, not at the outgoing cost alone
    ...
    #relayRouteThroughNeighbor(relay16: number, neighbor16: number, incomingCost: number, outgoingCost: number, now: number): void {
        ...
        // outright (#3.6.4.1). It is not a free link; it is no link.
        if (outgoingCost === 0) {
            return;
        }

        // 05-3474-23 #3.6.4.1: "The maximum of the incoming and outgoing costs for the neighbor is
    ...
    public async sendRouteReq(manyToOne: ZigbeeNWKManyToOne, destination16: number, destination64?: bigint): Promise<boolean> {
        ...
        const finalPayload = Buffer.allocUnsafe(1 + 1 + 1 + 2 + 1 + (hasDestination64 ? 8 : 0));
        let offset = 0;
    ...
    public async sendRouteReply(
        ...
        const finalPayload = Buffer.allocUnsafe(1 + 1 + 1 + 2 + 2 + 1 + (hasOriginator64 ? 8 : 0) + (hasResponder64 ? 8 : 0));
        let offset = 0;
    ...
    public processLinkStatus(data: Buffer, offset: number, macHeader: MACHeader, nwkHeader: ZigbeeNWKHeader): number {
        ...
        const options = data.readUInt8(offset);
        offset += 1;
        const firstFrame = !!((options & ZigbeeNWKConsts.CMD_LINK_OPTION_FIRST_FRAME) >> 5);
        const lastFrame = !!((options & ZigbeeNWKConsts.CMD_LINK_OPTION_LAST_FRAME) >> 6);
